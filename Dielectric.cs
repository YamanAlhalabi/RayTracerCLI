using System;
using OpenTK.Mathematics;

namespace SharpCanvas
{
    public class Dielectric : IMaterial
    {
        public Dielectric(float indexRefraction)
        {
            IndexRefraction = indexRefraction;
        }

        public float IndexRefraction { get; set; }

        public bool Scatter(Ray ray, HitRecord record, ref Color4 attenuation, ref Ray scattered)
        {
            attenuation = new Color4(1.0f, 1.0f, 1.0f, 1.0f);
            var refraction_ratio = record.FrontFace ? (1.0f / IndexRefraction) : IndexRefraction;

            var unit_direction = ray.Direction.Normalized();
            var cos_theta = MathF.Min(Vector3.Dot(-unit_direction, record.normal), 1.0f);
            var sin_theta = MathF.Sqrt(1.0f - cos_theta * cos_theta);

            var cannot_refract = refraction_ratio * sin_theta > 1.0;
            Vector3 direction;

            if (cannot_refract || Reflectance(cos_theta, refraction_ratio) > Helper.Random.NextDouble())
                direction = Helper.Reflect(unit_direction, record.normal);
            else
                direction = Helper.Refract(unit_direction, record.normal, refraction_ratio);

            scattered = new Ray(record.p, direction, ray.Time);
            return true;
        }

        private float Reflectance(float cosine, float ref_idx)
        {
            // Use Schlick's approximation for reflectance.
            var r0 = (1-ref_idx) / (1+ref_idx);
            r0 = r0*r0;
            return r0 + (1-r0)*MathF.Pow((1 - cosine),5);
        }
    }
}